...
class BookModel extends AbstractModel {
	...
	public function getAll(int $page, int $pageLength) : array
	{
		...
		$query = 'SELECT * FROM book LIMIT :page, :lenght';
		$sth = $this->db->prepare($query);
		$sth->bindParam('page', $start, PDO::PARAM_INT);
		$sth->bindParam('lenght', $pageLength, PDO::PARAM_INT);
		$sth->execute();
	...
	public function getByUser(int $userId): array {
		$query = "SELECT b. *
		FROM borroweb_books bb LEFT JOIN book b ON bb.book_id = b.id WHERE bb.customer_id = :id";
	...
	public function borrow(Book $book, int $userId) {
		$query = "INSERT INTO borrowed_books (book_id, customer_id, start) VALUES(:book, :user, NOW())";
			...
			$this->updateBookSock($book);
	}

	public function returnBook(Book $book, int $userId) {
		$query = "UPDATE borrowed_books SET end = NOW() WHERE book_id = :book AND customer_id = :user AND end IS NULL";
		$sth = $this->db->prepare($query);
		$sth->bindValue('book', $book->getId());
		$sth->bindValue('user', $userId);
		if (!$sth->execute()) {
